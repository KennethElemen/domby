                            ...
                            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                                ...
                                if ($uploadOk === 0) {
                                    ...
                                    if (move_uploaded_file($_FILES['img']['tmp_name'], $targetFile)) {
                                        ...
                                        $stmt->bind_param("ssssssssss", $roomNumber, $fullName, $email, $rate, $reference ,$paymentMethod, $paymentType, $imageData, $status , $paymentMonth);
            ...
            <div class="content-wrapper d-flex align-items-center justify-content-center auth">
                <div class="row flex-grow">
                    ...
                    <div class="text-center mt-3">
                        <h1 class="display-8">Monthly Payment</h1>
                        <br>
                    ...
                    <div class="col-lg-4">
                        <div class="auth-form-light text-left p-4">
                                ...
                                <label class="col-sm-6 col-form-label">Payment Type</label>
                                    <div class="col-sm-12">
                                        <input type="text" style="background-color: #edebeb;" class="form-control" name="payment_type" value="Monthly" readonly>
                                    </div>
                                </div>
                                <div class="form-group">
                                ...
                                if (isset($_SESSION['email'])) {
                                    // Get the logged-in EmailID
                                    $loggedInEmail = $_SESSION['email'];


                                    // Fetch MonthYear values where status is Pending for the logged-in user
                                    $sql = "SELECT DISTINCT MonthYear FROM paymentschedule WHERE EmailID = ? AND Status = 'Pending'";
                                    $stmt = $conn->prepare($sql);
                                    ...
                                    if ($result && $result->num_rows > 0) {
                                        ...
                                        echo '<select class="form-control" id="payment_Month" name="payment_Month" onchange="displayAmount(this.value)" required>';
                                        echo '<option value="">Select Month</option>';
                                        // Loop through the fetched data to create options
                                        while ($row = $result->fetch_assoc()) {
                                            $monthYear = $row['MonthYear'];
                                            // Format the MonthYear value if needed
                                            $formattedMonthYear = date('F Y', strtotime($monthYear)); // Assuming MonthYear is in YYYY-MM format

                                            // Create the option with the MonthYear value
                                            echo "<option value='$monthYear'>$formattedMonthYear</option>";
                                        }
                                        // Close the select element
                                        echo '</select>';
                                    } else {
                                        // Handle the case where no pending payments are found for the logged-in user
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" style="background-color: #edebeb;" class="form-control" id="rate" placeholder="" name="rate" value="" readonly />
                                </div>
                                <div class="form-group">
...
<div class="modal fade" id="Payment" tabindex="-1" role="dialog" aria-labelledby="termsAndAgreementModalLabel" aria-hidden="true">
    ...
    <script>
// Function to display the amount based on the selected MonthYear
// Function to display the amount based on the selected MonthYear
function displayAmount(selectedMonthYear) {
    // Check if a MonthYear is selected
    if (selectedMonthYear !== '') {
        // Get the logged-in email ID from the session
        var loggedInEmail = '<?php echo isset($_SESSION["email"]) ? $_SESSION["email"] : ""; ?>';

        // AJAX request to fetch the amount based on the selected MonthYear and logged-in email ID
        $.ajax({
            type: 'POST',
            url: 'fetch_amount.php', // Replace 'fetch_amount.php' with the actual PHP file to handle the AJAX request
            data: { monthYear: selectedMonthYear, email: loggedInEmail }, // Send the selected MonthYear and email ID as data
